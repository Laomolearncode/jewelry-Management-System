  ...
  <div class="page-card">
    <div class="page-header">
      <div>
        ...
        <p class="page-desc">支持多明细录单、审核和入库，完整覆盖采购主流程。</p>
      </div>
      <el-button v-if="canCreatePurchase" type="primary" @click="openDialog">新建采购单</el-button>
    </div>
    ...
    <div class="summary-grid">
      ...
      <div class="summary-item">
        <span>已完成</span>
        <strong>{{ statusCount.STOCKED }}</strong>
      </div>
    </div>

    <div class="toolbar">
      <el-input v-model="query.orderNo" placeholder="采购单号" clearable style="width: 180px" />
    ...
    <el-table :data="tableData.records || []" border>
      ...
      <el-table-column prop="status" label="状态" min-width="100">
        <template #default="{ row }">
          <el-tag :type="statusTagMap[row.status] || 'info'">{{ statusTextMap[row.status] || row.status }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" min-width="220">
        <template #default="{ row }">
          <div class="action-row">
            ...
            <el-button v-if="row.status === 'DRAFT' && canApprovePurchase" size="small" type="primary" @click="approve(row.id)">审核</el-button>
            <el-button v-if="row.status === 'APPROVED' && canStockInPurchase" size="small" type="success" @click="stockIn(row.id)">入库</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogVisible" title="新建采购单" width="1100px" top="4vh">
      ...
      <div class="toolbar">
        ...
        <el-tag type="success">预计金额 {{ estimatedAmount }}</el-tag>
      </div>
    ...
    <el-drawer v-model="detailVisible" title="采购单详情" size="55%">
...
import { listProductsApi, listSuppliersApi, listWarehousesApi } from '../../api/master'
import {
  approvePurchaseApi,
  createPurchaseApi,
  getPurchaseApi,
...
const canApprovePurchase = computed(() => hasPermission(authStore.user, 'purchase:order:approve'))
const canStockInPurchase = computed(() => hasPermission(authStore.user, 'inventory:stockin:create'))
...
async function approve(id) {
  await approvePurchaseApi(id)
  ElMessage.success('采购单审核成功')
  loadData()
}

async function stockIn(id) {
  await stockInPurchaseApi(id)
  ElMessage.success('采购单入库成功')
  loadData()
}

async function showDetail(id) {
  detailData.value = await getPurchaseApi(id)
